...
public class NextCalc {
	public ArrayList<String> nextcal(int month,int year){
		...
		if( month == 12 ) {
			next.set(Calendar.YEAR, year+1);				// 년도를 다음년도로 설정
			next.set(Calendar.MONTH, 0 );					// 달을 1월(0번) 로 설정
		}	
		else {
			next.set(Calendar.YEAR, year);
			next.set(Calendar.MONTH, month );				// 달을 다음달 번호로 설정 
		}
		next.set(Calendar.DATE, 1);							// 다음달을 1일로 설정
	
		int nextmonth =  next.get(Calendar.MONTH)+1;		// 다음달 가져오기
	
		int nextstartyoil = next.get(Calendar.DAY_OF_WEEK); // 다음달 시작요일 가져오기
